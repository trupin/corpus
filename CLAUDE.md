...
- `npm run e2e` — Playwright against the real Vite dev server (`apps/ui/e2e/`). It also collects browser-side V8 coverage for the merged gate. The dev server's port is `5173` by default; override with `CORPUS_UI_PORT` when something else holds it (`.githooks/pre-push` defaults it to `5273` so a push never fights a running dev server). **The suite cannot reach a workspace server** (INFRA-028): it starts Vite with no proxy target, so `/api`, `/attachments` and `/events` are refused inside the dev server. A workspace server running on `8765` no longer changes the result, and a local failure is no longer explainable as "that pair needs 8765 free"
