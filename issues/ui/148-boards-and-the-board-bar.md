...
- [ ] Which board shows is browser-local and survives reload; a browser that remembers no board shows the `default-open` board, else the first in `order` (§10 amendment, 2026-08-22); a remembered board that is archived or deleted falls back the same way.
