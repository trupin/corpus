...
- [ ] Tree: `useTree()` for folders with counts; a folder's documents load on expand via `GET /api/docs?folder=<path>&limit=…` with the §10 "listing reached its bound" treatment past the limit; collapse state local; `.claude/skills` appears as the tree already reports it.
