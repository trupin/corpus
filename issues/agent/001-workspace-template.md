...
- SPEC.md §10 (seed columns, templates) — columns are pinned `type: view` documents with `order`; the seed ships Attention / Inbox / Open threads, deletable like any document; templates are `type: template` documents with `for: <doc-type>`
...
**Seed view shape.** A column IS a `type: view` document (§10). Its query lives under a `query:` mapping whose keys mirror the `GET /api/docs` parameters of §9.2 verbatim — `needs`, `folder`, `type`, `status`, `tag`, `q`, `sort` — so the UI can hand the mapping to the collection endpoint without translation:
...
`folder` values are paths relative to `data/docs/` with no leading or trailing slash (`inbox`). If SHARED-001's revised §9.2/§10 pins different names or a different nesting for the query, follow the spec and note the deviation in the issue — the spec wins.
...
- **Skill command accuracy against `docs/cli.md` → DEFERRED → AGENT-002/003.** `docs/cli.md`
  ...
  names literally (§2.1, §7, §12). Two shapes were the skeletons' expectations of the CLI
...
#### 2026-07-26 — Install contract made exhaustive for CLI-002 (sprint-003 Open Conflicts 9 and 10)

CLI-002's `corpus init` generated eight things the install contract did not record. The
orchestrator adjudicated that "Generated by `corpus init`, not copied" **is meant to be
exhaustive** — an install contract recording only part of what `init` creates cannot serve as
CLI-005's three-way-compare baseline or as the reader's ground truth. `INIT_GENERATED` in
`scripts/workspace-template.ts` and the doc's bullet list now carry all eleven entries,
byte-identical and in the same order (the existing test compares them), with the directory
order matching `WORKSPACE_DIRECTORIES` in `apps/cli/src/commands/init/scaffold.ts`:

```
data/docs/inbox/  data/threads/  .claude/skills-archived/  .claude/agents/
.corpus/queue/  .corpus/locks/  .corpus/jobs/  .corpus/attachments/
.corpus/config.json  .corpus/template-manifest.json  git init
```

- The four directories the template cannot deliver — `data/docs/inbox/`, `data/threads/`,
  `.claude/agents/` hold nothing but a `.gitkeep`, which the copy filter drops;
  `.claude/skills-archived/` has no template counterpart at all. Three of them are projection
  document roots (Conflict 9, generalized past the two roots it named).
- The three runtime directories `.corpus/locks/`, `.corpus/jobs/`, `.corpus/attachments/`
  (SPEC.md §4), created empty and filled by the server later.
- `.corpus/template-manifest.json` — shape pinned as a fenced `json` block matching what
  `scaffoldWorkspace` writes: `{version: 1, tool, installedAt, files: [{path, sha256}]}`,
  `files` holding post-rename workspace-relative paths in install order (Conflict 10).

The lead paragraph now states the exhaustiveness claim and why it matters, notes that
directories filled by the copy (`data/docs/templates/`, `data/docs/views/`, `.claude/skills/`
and their parents) are deliberately absent, and records that `cache.db`, `server.pid`,
`server.log`, `seen.json` and `HALT` are server- or lifecycle-created rather than
`init`-created.

`scripts/workspace-template.test.ts` gained eight cases. Four are structural: the kind
partition (directories / files / the single `git init` action), path well-formedness
(relative, no `..`, no backslash), copy-vs-generate disjointness, and the manifest shape
parsed as real JSON and compared key by key. Four hold the exhaustiveness claim:

- **Generalized filter rule** — every template directory whose entire content is dropped by
  the copy filter must appear as a generated directory. The template directory is mapped to
  its installed form by running an unfilterable probe name through the real `installedPath()`
  rather than re-deriving the rename prefixes.
- **Doc vs. implementation** — the test imports `WORKSPACE_DIRECTORIES` from
  `apps/cli/src/commands/init/scaffold.ts` (read-only; it typechecks and runs cleanly from
  `scripts/`, which already has the reverse import in `apps/cli/.../template.test.ts`). Every
  directory `init` creates must be accounted for: filled by the copy, containing a listed
  entry, or being/inside a listed directory. Nothing may be listed that `init` never creates.
  The listed directory order must equal `init`'s creation order.

The covering rule was negative-checked before being relied on: dropping `.corpus/jobs/`
reports `['.corpus/jobs']`, dropping `data/threads/` reports `['data/threads']`, dropping
`.corpus/queue/` reports the queue directory and all five status directories. It is
directory-scoped by construction — a missing generated *file* is caught by the doc-vs-module
equality test and the manifest-shape test instead.

```
$ npx vitest run scripts/
 PASS (49) FAIL (0)          # workspace-template.test.ts: 43

$ npx prettier --check docs/workspace-template.md scripts/workspace-template.{ts,test.ts} \
    issues/agent/001-workspace-template.md
All matched files use Prettier code style!

$ npx eslint scripts/workspace-template.ts scripts/workspace-template.test.ts   # clean
$ (cd scripts && npx tsc --noEmit -p tsconfig.json)                             # clean
```

Model: opus. `apps/cli` was read but not modified.
