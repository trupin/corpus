...
- [ ] **Nullable timestamps decision** _(SERVER-011 handoff, 2026-07-27)_: `documents.created/updated` are legitimately null (hand-written skill files) but `DocRow` declares both non-nullable — the server currently serializes an epoch sentinel and staleness treats unknown age as fresh. Decide: make the row fields nullable (UI renders "—") or bless the sentinel; either way document it in the schema description.
