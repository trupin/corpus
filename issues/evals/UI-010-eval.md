...
| 143 | Legend covers §10 item by item              | PASS   | All twelve prototype rows present and correctly worded: `↑ ↓ move rows (also j / k)`, `↵ open document`, `⇧↵ open in full screen`, `esc close / back`, `← → switch column (also [ / ])`, `⇧← ⇧→ move column`, `f focus mode`, `e archive open / highlighted doc`, `r reply in open thread`, `c Ask / Capture composer`, `⌘K search`, `? this cheat-sheet`. Nothing present in one and absent from the other |
...
| 145 | Every binding does what §10 says            | PASS   | `j`/`k` and `↑`/`↓` move the `.row.kbd` cursor in the active column; ten `k` presses **clamped** at the first row (no wrap); `↵` opened the reader (focus-mode count 0); `⇧↵` opened focus mode directly; `f`, `e`, `r`, `?` all verified |
