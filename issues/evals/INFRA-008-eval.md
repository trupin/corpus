...
| 83  | `README.md` documents contributor setup          | PASS                          | Contributing section: clone → `npm install` → **`npm run setup-hooks`** (called out as the required one-time step, §15) → `npm run build` → `npm test`, plus the per-phase-PR / green-CI / **squash-only** policy. |
