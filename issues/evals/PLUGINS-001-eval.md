...
| 135| The scoped-template-keys question is filed, not answered | PASS | Quoted **verbatim** in the log as an open question for user sign-off, framed against the SERVER-005 template-bleed fix and the §10-vs-§9.2 tension. **No mechanism was built**: `seedTemplate` is documented as supplying a **BODY only** (`docs/PLUGINS.md:87`, "optional, a BODY-only template (§9.2)") and appears only in the YAML schema and the parity test — no code path moves template frontmatter onto instances. Adjudication 3 held in both directions. |
