...
| 25  | Rebuild is idempotent (§12 M1)                       | PASS         | Two rebuilds into two temp paths; all twelve tables dumped with deterministic ordering (253 lines) → `diff` identical. Only `meta.rebuilt_at` differs.                                          |
