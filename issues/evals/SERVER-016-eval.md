...
| TEST-89  | §11 warnings ride the response                   | PASS   | Accepted on the log's evidence (`201` + non-empty `commit_failed` warning, `HEAD unchanged: True`, turn still on disk). Not independently re-run — a pre-commit hook was not installed in the eval workspace.            |
