...
- [x] §10's "Visual reference" line names `design/navigation.html` as authoritative for navigation look and feel, beside `design/index.html` for everything else.
- [x] No sentence in §10 still says "No sidebar", "pinned: true", "column widens", or "order (board position)" about a view.
...
**Rider 1 — §10, the explorer (replaces "No sidebar").**
...
**Rider 2 — §10, boards are documents (replaces "Columns are pinned view documents").**
...
**Rider 3 — §10, paths (replaces "Clicking a row opens the document _in that column_ (column widens)").**
...
- Strike the replaced sentences in the same edit as the rider that replaces them, so §10 never says both.
...
### Verification Steps
1. `grep -n "No sidebar\|pinned: true\|column widens" SPEC.md` returns nothing in §10.
...
1. §10 explorer — replaced "No sidebar."; the Shell sentence names the explorer; the Visual reference line names `design/navigation.html`.
2. §10 boards as documents — replaced "Columns are pinned view documents" through "nothing hardwired"; the view's query/stewardship sentence kept and reworded.
3. §10 paths — replaced "Per-column reader"; three phrases amended (creation "in a path off its column", console "loose path at the left edge", keyboard `↵`/`⌥↵`/`esc` order/`⇧esc`).
4. §10 column strip — inserted in "The board."; the ghost column now "creates view documents and adds them to the board".
5. §5 `stage` — frontmatter line added after `evergreen`; coupling paragraph after the 2026-08-12 unarchive rider.
6. §10 kanban — new bullet after "Folder scoping".
7. §9.2 — `pinned=` out, `stage=` in on `/api/docs` and `/api/search`; the 2026-08-08 amendment reworded; folder acts line after `GET /api/tree`; board fields in the `POST /api/docs` line. Consequences applied under it: §9.1 projection columns (`stage`, `board_json`, no `pinned`), the plugin section's column sentence (that section was deleted by SHARED-067), M4 milestone line.
