...
**Reversed by `AGENT-063`, 2026-09-02, user directive** (*"default to: orchestrator
picks based on the task"*): the fixed strongest-tier default this issue shipped is
gone — a weightless designation is now judged on the conversation. The §7 rider
this issue drafted was withdrawn unsigned, not signed, and its section is deleted.
The half that survives: the two-pass job judgment does not govern a listener
launch, and every launch logs its weight and that weight's provenance.
