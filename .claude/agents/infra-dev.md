...
- **2026-07-28 — Version singularity.** Root `package.json` `version` is the single source (added by INFRA-008; it had none). `scripts/check-versions.ts` asserts root == every workspace and, when `GITHUB_REF` names a `refs/tags/v*`, that the tag agrees — one script is both the drift check and the release guard. It is pre-push's **first** step (cheap, no build); `pack:check` is CI-only (it bundles and packs).
- **2026-07-28 — `.githooks/pre-push` has always been able to block.** sprint-013 Open Conflict 7 claimed it had no exit epilogue and could not fail a push. `git show 6e7e709:.githooks/pre-push` disproves it: the `if [ "$fail" -ne 0 ]; then exit 1; fi` epilogue and the Playwright step have been there since INFRA-001. Proven both directions by running the real hook with `npm`/`node` stubbed on `PATH` (`FAILING_STEP=<script>`), which needs no clone and changes no git state — reuse that technique.
- **2026-07-28 — Packaged plugins are not actually loadable yet (PLUGINS-002 owns the fix).** Two gaps, found while writing the pack rule and recorded in `stagePlugins`: (1) `discoverPluginTopics` enumerates `<plugin>/cli/commands/*.ts` — the **source** dir — to learn command names before mapping to `dist/`, so a dist-only packaged plugin exposes no CLI commands; (2) a packaged plugin's `dist/server/routes.js` imports `@corpus/contract` as a bare specifier, which is inlined into the tool's bundles rather than installed, so it will not resolve. Bundling each plugin during staging is the natural fix and belongs in `scripts/package-staging.ts`.
