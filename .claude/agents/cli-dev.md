...
- **2026-09-05 — Every help page is byte-budgeted at registry load (CLI-080).** `validateRegistry` renders every verb and topic page through `help.ts` (`color: false`) and refuses brief > 1,600 B or full > 8,000 B (`HELP_BUDGET_BYTES`) — so a new flag on a hot verb (`doc edit` sits at ~7,9xx/1,470, `doc list` brief at 1,596) fails the build until prose elsewhere on that page pays for it; the fix is compression, never an exemption. Three consequences to know before editing descriptions: (1) the glossed global-flags block lives **only** on `corpus --help` — verb/topic full pages carry a names-only line, brief carries no globals at all; (2) three sentences are test-pinned (`validate.test.ts` "silent-damage") because the orchestrate skill cites them as the only warning: `doc edit --stage`'s same-commit status write, `doc create --folder`'s validated-then-no-effect under `--type thread`, `doc edit --columns`'s `""`-vs-`--unset` distinction — rephrase around them, never through them; (3) the compression doctrine is: a fact a refusal or printed warning states at the moment it matters may be a clause, a fact that fails silently keeps its sentence. `", repeatably"` in a repeated flag's prose duplicates the renderer's `(repeatable)` suffix — omit it.
...
- Cap workers on every vitest invocation: `VITEST_MAX_WORKERS=4`.
