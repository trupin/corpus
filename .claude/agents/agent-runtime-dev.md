...
- **2026-08-02 — Skill-text tests constrain where a new rule can go (AGENT-010).** `scripts/workspace-template.test.ts` asserts `sections.size` EXACTLY (comment 13, orchestrate 15) and every `## ` section must exceed 400 chars — a new convention belongs inside an existing section unless the issue genuinely warrants updating those counts. Also: never nest a fence inside a ` ```bash ` example — the template's fence-toggling extractor and the section parser both mis-track it; show deliverable fences at section level (the way the `form` grammar already does).
