...
**There is no `--key`** (SPEC.md §7). A patch names the text it expects to find, which is the same staleness check by another route — and a better one, because it says _which_ text is gone rather than _that_ the document changed. Everything else about a patch is an ordinary write: it is validated before it lands (SPEC.md §14), anchors are reconciled with remaps and orphans reported (SPEC.md §6), the change is committed and attributed to `--from` — folded into the open commit window like any other save, not necessarily a commit of its own (§4) — and the fresh key is printed on the line after the confirmation.
