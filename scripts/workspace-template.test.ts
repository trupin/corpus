...
/** The orchestrate skill's references (AGENT-067): case law, litigation, procedures. */
const orchestrateReferences = referencesOfSkill("orchestrate");
/** The reflect-edit skill's references (AGENT-067): reasoning and the worked example. */
const reflectEditReferences = referencesOfSkill("reflect-edit");
const skillReferences: readonly InstalledSkill[] = [
  ...commentReferences,
  ...converseReferences,
  ...orchestrateReferences,
  ...reflectEditReferences,
];
...
/**
 * The orchestrate skill as its agent can read it (AGENT-067): the body plus the
 * five references its sections point at conditionally. A pin asking "does the
 * orchestrate skill state this?" reads this package; a pin about *where* a rule
 * sits — the tier table above all — reads the one file.
 */
const orchestratePackage = [
  documentAt("claude/skills/orchestrate/SKILL.md").body,
  ...orchestrateReferences.map(({ body }) => body),
].join("\n\n");

/** The reflect-edit skill as its subagent can read it (AGENT-067), same terms. */
const reflectEditPackage = [
  documentAt("claude/skills/reflect-edit/SKILL.md").body,
  ...reflectEditReferences.map(({ body }) => body),
].join("\n\n");

/** One orchestrate reference file's text, by basename — for placement pins. */
const orchestrateReference = (name: string): string => {
  const reference = orchestrateReferences.find(({ label }) => label.endsWith(`/${name}`));
  if (reference === undefined) throw new Error(`no orchestrate reference named ${name}`);
...
describe("skills", () => {
   ...
   * `references/` files its body points at, and AGENT-067 did the same for
   * `orchestrate`, so a pin asking "does the skill still teach this?" reads the
   * package. Pins about *placement* keep naming one file.
   */
  const skillDoctrine = [
    { name: "orchestrate", text: orchestratePackage },
  ...
  it.each(coreSkills)("$name carries its required section headings", ({ name, relPath }) => {
    ...
    const requiredBySkill: Record<string, readonly string[]> = {
      // AGENT-067: "writing a document" and "worked example" left this list —
      // both live whole in `orchestrate/references/` now, pinned by the
      // reference describes; the body keeps a conditional pointer to each.
      orchestrate: [
        "purpose",
        "invariants",
        "command's help",
  ...
  describe("trace lines", () => {
    // Doctrine pins read each skill's package (AGENT-067): orchestrate's worked
    // turns live in its references now, and the rule's statement stays in the
    // body's stewardship charter either way.
    it.each(skillDoctrine)("$name states the trace grammar", ({ text }) => {
      expect(text).toContain("↳ ");
      expect(text).toMatch(/past-tense/i);
      expect(text).toMatch(/final line — and only its final line —/);
      expect(text).toMatch(/changed nothing/i);
    });

    it.each(skillDoctrine)("$name puts a trace last, or not at all", ({ name, text }) => {
      const lines = text.split("\n");
      const traceLines = lines.filter((line) => line.trimStart().startsWith("↳"));
      expect(traceLines.length, `${name}: no worked-example trace`).toBeGreaterThan(0);
      for (const [index, line] of lines.entries()) {
        if (!line.trimStart().startsWith("↳")) continue;
        // Every trace written into an example turn is that turn's last line:
        // the heredoc terminator is what comes next.
        expect(lines[index + 1]?.trim(), `${name}: trace not last in its turn`).toBe("CORPUS_EOF");
      }
    });

    it.each(skillDoctrine)("$name neither hides the arrow nor dresses it up", ({ text }) => {
      ...
      expect(text).not.toMatch(/omit the arrow/i);
      expect(text).not.toMatch(/::before/);
      expect(text).not.toMatch(/<span[^>]*>\s*↳/);
    });
    ...
    it("keeps traces out of user-authored turns", () => {
      ...
      for (const { name, text } of skillDoctrine) {
        const userTurns = text
          .split("\n")
          .filter((line) => /^(?:user|##\s*user)\b/i.test(line.trimStart()));
        for (const line of userTurns) expect(line, name).not.toContain("↳");
      }
      // Both skills' traces live inside `--from agent` reply heredocs only.
      expect(orchestratePackage).toMatch(/nothing changed, so that reply carries no trace line/);
    });
  });
  ...
  describe("deliverable fences", () => {
    // Doctrine pins read the packages (AGENT-047/AGENT-067): the full canvas
    // and fence statements sit in comment's body and orchestrate's
    // `references/writing.md`, and the placement pin below reads the one file.
    it.each(skillDoctrine)("$name states the labeled-fence convention", ({ text }) => {
      expect(text).toMatch(/info string/);
      expect(text).toMatch(/copyable canvas/);
      expect(text).toMatch(/one deliverable per fence/i);
     ...
     */
    it.each(skillDoctrine)("$name states both halves from one mechanism", ({ text }) => {
      expect(text).toMatch(/closes only on a line that is nothing but backticks/i);
      expect(text).toMatch(/wider than anything inside/i);
      expect(text).toMatch(/longest backtick run in the payload/i);
      expect(text).toMatch(/on a line of its own/i);
      // The consequence, never the mere fact of malformed markup.
      expect(text).toMatch(/closes nothing/i);
      expect(text).toMatch(/heading \*{0,2}inside a fence is (?:deliberately )?not a delimiter/i);
      expect(text).toMatch(/swallow/i);
      expect(text).toMatch(/no error anywhere|nothing anywhere reports an error/i);
    });
    ...
    it("keeps orchestrate's copy deferential rather than a second statement of the rule", () => {
      ...
      expect(body).toMatch(wrapped("The comment skill states the convention"));
  ...
  describe("stating the model that wrote the turn", () => {
    ...
    it("owns the stamp-repair prohibition, in the comment skill alone (AGENT-061)", () => {
      ...
      const orchestrate = orchestratePackage;
      expect(orchestrate).toMatch(wrapped("The comment skill states the grammar"));
  ...
  describe("a key, not a lock", () => {
    ...
    it.each(skillDoctrine)("$name works a body-replacing edit at all", ({ text }) => {
      ...
      expect(bodyReplacingEdits(text).length).toBeGreaterThan(0);
    });
    ...
    it.each(skillDoctrine)("$name teaches the loop rather than a rule to recall", ({ text }) => {
    ...
    it.each(skillDoctrine)("$name answers a stale key concretely", ({ text }) => {
      const body = text;
    ...
    it.each(skillDoctrine)("$name treats the editing signal as a courtesy", ({ text }) => {
      const body = text;
    ...
    it("keeps the delta verbs free of a key, in both skills", () => {
      for (const { name, text } of skillDoctrine) {
        expect(text, `${name}: no delta rule`).toMatch(/names its own delta/);
      }
      expect(orchestratePackage).toMatch(/--add-tag/);
    });
  });
  ...
  describe("a bounded change is a patch, not a rewrite", () => {
    ...
    it.each(skillDoctrine)("$name states the choice as one rule, with both costs", ({ text }) => {
      const flat = text.replace(/\s+/g, " ");
      expect(flat).toMatch(
    ...
    it.each(skillDoctrine)("$name presents no key on a patch, and says why", ({ text }) => {
      const body = text;
     ...
     */
    it.each(skillDoctrine)("$name says a patch replaces and does not insert", ({ text }) => {
      const flat = text.replace(/\s+/g, " ");
    ...
    it.each(skillDoctrine)("$name matches byte for byte, body only", ({ text }) => {
      const flat = text.replace(/\s+/g, " ");
    ...
    it.each(skillDoctrine)("$name keeps the two refusals apart", ({ text }) => {
      const flat = text.replace(/\s+/g, " ");
    ...
    it("keeps the changelog append on the keyed whole-body path, and says why", () => {
      ...
      const skill = reflectEditPackage;
  ...
  describe("a person's words never reach the shell as a literal", () => {
    ...
    it("gives one construction and the test for when it applies", () => {
      const flat = orchestrate.replace(/\s+/g, " ");
      // The construction is a **path** now (CLI-074, AGENT-058). What it
      // replaced — a value captured out of a quoted heredoc — was correct for
      // every character and wrong about one line, and the line was the person's
      // to choose. The pins below moved with it; the two properties underneath
      // them did not.
      ...
      // one step earlier rather than removing it.
      expect(flat).toMatch(/write it to a file with your file-writing tool/i);
      expect(flat).toMatch(/\*\*Do not build the file with a shell either\.\*\*/);
      ...
      // One rule covering a flag and a body alike, which is what kept this from
      // becoming two rules that agree until one is edited.
      expect(flat).toMatch(
        /One rule, and the same one whether the words fill a document or a single flag/i,
      );
    });
    ...
    /**
     * **Retired with the construction it repaired** (AGENT-058), and this
     * docblock is what stops it coming back.
     *
     * The clause it pinned handed the agent `IFS= read -r` after a parse error,
     * because the *capture* form — a quoted heredoc inside `$( … )` — is
     * refused by bash 3.2 on one unbalanced apostrophe, quote or backtick
     * anywhere in the value. Measured 2026-08-18 across both shells; the
     * measurement stands and is why nothing captures a value any more.
     *
     * With no capture there is no parse error to repair, and `IFS= read -r`
     * would be worse than nothing if it stayed: it takes **one line** and drops
     * the rest at exit `0`, which is a silent truncation offered as a remedy.
     * The heredocs that remain are bodies the agent wrote, fed straight to a
     * command with no substitution around them, so they cannot reach the failure
     * either.
     *
     * What replaces the pin is a prohibition: neither repair may reappear
     * without the capture form reappearing first, and that is what the test
     * below asserts.
     */
    it("no longer captures a value, so it offers no repair for capturing one", () => {
      // The construction is gone from every skill, not only from the prose that
      // used to describe it.
      for (const { label, body } of installedSkillTexts) {
        expect(body, `${label}: captures a flag value out of a heredoc`).not.toMatch(
          /\w+=\$\(cat <<'CORPUS_EOF'/,
        );
      }
      // And so is the repair, which truncates at one line and would be a silent
      // data loss offered as a remedy.
      expect(orchestrate, "the one-line repair outlived the failure it repaired").not.toMatch(
        /IFS= read -r/,
      );
    });
     ...
     */
    /**
     * The residual **became the reason** (AGENT-058). It was the one thing the
     * old construction could not cover — a line, not a character — and the skill
     * carried it as a caveat with a one-word fix. Measured again on 2026-08-30
     * against v0.28.0, the caveat is not survivable by choosing a better word:
     * a pasted vendor transcript containing the terminator ran `touch`, exited
     * `0`, and committed a document holding a command's own output in place of
     * the missing lines, tail intact.
     *
     * So it is no longer a caveat on the rule; it is the argument for it, and
     * the skill has to make that argument or an agent reads the path form as
     * ceremony and goes back to the heredoc it can see working.
     */
    it("makes the failure it removes the reason for the rule, not a caveat on it", () => {
      const flat = orchestrate.replace(/\s+/g, " ");
      // The totality claim survives — qualified to characters, which is what it
      // was always true of.
      expect(flat).toMatch(/there is no character list to keep in your head/i);
      // The failure, named as the reason the construction changed.
      expect(flat).toMatch(/\*\*Why not a heredoc, which is what this skill used to say\.\*\*/);
      expect(flat).toMatch(/one failure that is not about characters at all/i);
      ...
      expect(flat).toMatch(/runs the remainder \*\*as commands\*\*/i);
      expect(flat).toMatch(/exited `0`/i);
      expect(flat).toMatch(/with the tail of the message intact, so nothing read as truncated/i);
      // And why inspection is not the way out, which is the sentence that stops
      // an agent deciding per message whether this one is safe.
      expect(flat).toMatch(/you cannot inspect your way out of that/i);
      // The property that makes a path different in kind rather than in degree.
      expect(flat).toMatch(/A path has no terminator, so it has no line that can end it/i);
    });

    it("fixes the terminator once, for the heredocs that remain", () => {
      const flat = orchestrate.replace(/\s+/g, " ");
      // A heredoc is still right for words the agent wrote, and those still
      // need a terminator — so the rule survives, scoped to them.
      expect(flat).toMatch(/\*\*Where a heredoc is still right\.\*\*/);
      expect(flat).toMatch(/Words \*you\* wrote/);
      // Unconditional, and naming both words: the one to use and the one the
      // rest of the world's transcripts already end with.
      expect(flat).toMatch(/the terminator is always `CORPUS_EOF`, never `EOF`/i);
      ...
      expect(flat).toMatch(/choosing per message is the weighing this rule exists to remove/i);
    ...
    it("has heredocs in the installed skills for that rule to bind", () => {
      ...
      // A floor, not the count: the pin is that the rule above is checking real
      // examples. It read 35 until INFRA-031 deleted the todos plugin's own
      // skill, then 34; AGENT-058 took it to 30 by moving every **carried**
      // value out of a heredoc and onto a path. What is left is bodies the agent
      // wrote itself, which is what the rule now says a heredoc is for — so the
      // floor drops on purpose and the anti-vacuity still bites.
     ...
     */
    it("writes a thread's title to a file, at both sites that set one", () => {
      ...
      const retitles = [...comment.matchAll(/corpus doc edit th_\w+ (--\S+)/g)].map(
        (match) => match[1],
      );
      expect(retitles.length, "the comment skill retitles no thread").toBeGreaterThan(1);
      for (const argument of retitles) {
        // Not `--title "…"`, and not `--title "$title"` either: the value is
        // named by path, so no shell reads it on the way in.
        expect(argument, "a thread title reaches the command as an argument").toMatch(
          /^--flag-file$/,
        );
      }
      // Every retitle names a title file. Not an equality: the skill also tells
      // the agent to create a document by path, which is the same rule at a
      // different verb and would make the count say something it does not mean.
      expect(comment.match(/--flag-file title=\S+/g)?.length ?? 0).toBeGreaterThanOrEqual(
        retitles.length,
      );
    });
    ...
    it("passes a skill's description by path, like any other prose somebody reads", () => {
      ...
      for (const invocation of comment.match(/corpus skill create [^\n]*/g) ?? []) {
        expect(invocation, "a skill description is quoted straight into the command").toMatch(
          /--flag-file description=\S+/,
        );
        expect(invocation, "a skill description is still a shell argument").not.toMatch(
          /--description /,
        );
      }
    });
...
describe("orchestrate skill body", () => {
  // The package (AGENT-067): doctrine that moved into `references/` is still
  // this skill's doctrine. Pins about the file's own structure read
  // `skillFile`; section-scoped pins read the section plus the reference its
  // pointers name, since that pair is what a pass on that section can read.
  const body = orchestratePackage;
  const skillFile = documentAt("claude/skills/orchestrate/SKILL.md").body;
  const launchingReference = orchestrateReference("launching.md");
  const conventionsReference = orchestrateReference("conventions.md");
  const workedExampleReference = orchestrateReference("worked-example.md");
  ...
  it("gives every section a substantive body, not a bare heading", () => {
    ...
    // 15 since AGENT-067 moved *Writing a document* and the worked example into
    // `references/` whole. Was 17 since AGENT-066, 19 since AGENT-051.
  ...
  describe("weighing a dispatch", () => {
    ...
    it("subsumes the reflection's ad-hoc tier rule into the general test", () => {
      ...
      const procedure = reflectEditPackage;
  ...
  describe("a weight the request stated", () => {
    ...
    it("does the work anyway when a stated weight cannot be met, and says so twice", () => {
      ...
      expect(body).toMatch(
        // AGENT-067: the sentence lives in `references/weight.md`, which names
        // the tier table instead of pointing "above" at it.
        wrapped("the level is the word a reader can hold against the tier table"),
      );
  ...
  describe("the loop is a procedure, not a script", () => {
    ...
    it("keeps the empty-batch pass a two-command pass as well", () => {
      const claiming =
        body.slice(body.indexOf("## Claiming"), body.indexOf("## Routing")) +
        "\n\n" +
    ...
    it("dispatches between the claim and the park in the worked example too", () => {
      const example = workedExampleReference;
  ...
  describe("sharing the queue", () => {
    const routing =
      body.slice(body.indexOf("## Routing"), body.indexOf("## Delegation")) +
      "\n\n" +
      launchingReference;
    const claiming =
      body.slice(body.indexOf("## Claiming"), body.indexOf("## Routing")) +
    ...
    it("launches once per lane per pass, and stops when a launch does not take", () => {
      ...
      /*
       * AGENT-055 made it three fields. `working` is the one easy to leave out
       * and the one that costs an agent when you do: a resident working inline
       * holds no park, so a long turn reads exactly like a dead lane and a
       * launch puts a second listener on a conversation already thinking.
       */
      expect(routing).toMatch(
        /does not read `live`, \*\*has something pending\*\*, and is\s+\*\*not working\*\*/,
      );
     ...
     */
    /**
     * AGENT-055's own guard. The tolerance for duplicates is the **fallback
     * position**, not the rule, and it must survive: `working` can be stale for
     * the same reason `live` can, so launching under what is left of the
     * uncertainty is still right.
     */
    it("keeps launching under uncertainty right, and says why the asymmetry decides it", () => {
      expect(routing).toMatch(/a duplicate resolves itself at\s+the first message/);
      expect(routing).toMatch(/launching costs a wasted session, occasionally/);
      // The comparison that settles it, in the terms v0.23.0 made true.
      expect(routing).toMatch(/it is not answered/);
      expect(routing).toMatch(
        /`working` narrows this rule and does not reverse it|narrows this rule and does not reverse it/,
      );
      expect(routing).toMatch(
        /a wasted session against an unanswered conversation is not\s+a close call/,
      );
    });

    it("knows a not-live row is ambiguous, launches anyway, and invents no separator", () => {
      /*
       * **AGENT-055 narrowed this rule and did not reverse it**, which is the
       * distinction the whole bullet now turns on. `working` answers one of the
       * three things a not-live row could mean; the other two — a crashed
       * listener and one unobserved since a restart — are still
       * indistinguishable, and still want the same thing.
       */
      expect(routing).toMatch(
        /\*\*A row that does not read `live` still does not mean nobody is there/,
      );
      expect(routing).toMatch(/\*\*and for one in the middle of a\s+turn\*\*/);
      expect(routing).toMatch(/indistinguishable\s+from an empty lane \*by presence alone\*/);
      expect(routing).toMatch(
        /\*\*`working` separates the third of those, and only the third\.\*\*/,
      );
      expect(routing).toMatch(/it does not need to, because both want the same thing/);
      // The forbidden separators survive by name — each is a plausible "fix",
      // and answering one uncertainty must not read as licence to probe for the
      // rest.
      expect(routing).toMatch(/\*\*Everything the old argument forbade, it still forbids\.\*\*/);
      expect(routing).toMatch(
        /no probe, no holding back a pass to see what happens, and above all no reading the/,
      );
      expect(routing).toMatch(/whose length is promised and whose content\s+is\s+not/);
      ...
      expect(routing).toMatch(/\*\*launch, and let the lane settle it\.\*\*/);
      expect(routing).toMatch(
    ...
    it("says which lane a quoted mention wakes, rather than assuming its own", () => {
      ...
      const reflecting = reflectEditPackage;
    ...
    it("tells the operator how a broken converse skill looks, and how to put it back", () => {
      const recovery =
        body.slice(body.indexOf("## If the loop breaks"), body.indexOf("\n# Launching")) +
        "\n\n" +
        launchingReference;
...
describe("comment skill body", () => {
  ...
  it("makes the standalone title an obligation, set through a document edit", () => {
    expect(body).toMatch(/corpus doc edit th_\w+ --flag-file title=\S+ --from agent/);
  ...
  it("states skill genesis: threshold, destination, mechanism, announcement, conflicts", () => {
    ...
    expect(genesis).toContain(
      "corpus skill create <name> --flag-file description=<path> --from agent",
    );
...
describe("progressive disclosure in the comment skill (AGENT-047)", () => {
  ...
  it("gives no reference frontmatter, and excludes each for that reason", () => {
    // 7 comment references (AGENT-047), converse's leaving.md (AGENT-065),
    // orchestrate's five and reflect-edit's two (AGENT-067).
    expect(skillReferences).toHaveLength(15);
...
describe("the event procedures live in skills of their own (AGENT-066)", () => {
  ...
  it("records the drop in bytes, so the text cannot quietly move back", () => {
    // 167,737 bytes before AGENT-066, 139,824 after it, and ~50,000 after
    // AGENT-067's split. The cap leaves headroom for ordinary editing and none
    // for reinstating a moved procedure or section: the smallest moved piece
    // (the worked example) alone is about 4,800 bytes.
    expect(Buffer.byteLength(orchestrate, "utf8")).toBeLessThan(51_500);
  });
});

/**
 * AGENT-067 — the orchestrate split, and reflect-edit's shrink (AGENT-066's
 * recorded debt). The body keeps what an ordinary dispatch-only pass reads —
 * the loop, claiming, the routing table and its decisions, the two passes and
 * the tier table, the log grammar, settlement — and five `references/` files
 * carry what a minority of passes reads, each reachable through a conditional
 * pointer. The guards here are the split's own: the files exist, the pointers
 * and files pair up, the tier table stays in the one file the composer and the
 * CLI parse, and the sizes cannot quietly regrow.
 */
describe("the orchestrate split (AGENT-067)", () => {
  const skillFile = readTemplateFile("claude/skills/orchestrate/SKILL.md");
  const reflectEditFile = readTemplateFile("claude/skills/reflect-edit/SKILL.md");
  const basenames = (references: readonly InstalledSkill[]): string[] =>
    references.map(({ label }) => label.split("/").at(-1) ?? "").sort();

  it("ships orchestrate's five references and reflect-edit's two", () => {
    expect(basenames(orchestrateReferences)).toEqual([
      "conventions.md",
      "launching.md",
      "weight.md",
      "worked-example.md",
    ...
    expect(basenames(reflectEditReferences)).toEqual(["reasoning.md", "worked-example.md"]);
  });

  it("pairs every pointer with a file and every file with a pointer", () => {
    for (const [file, references] of [
      [skillFile, orchestrateReferences],
      [reflectEditFile, reflectEditReferences],
    ] as const) {
      const named = new Set(
        [...file.matchAll(/references\/([a-z-]+\.md)/g)].map((match) => match[1] ?? ""),
      );
      for (const name of basenames(references)) {
        expect(named, `no pointer in the body names references/${name}`).toContain(name);
      }
      for (const name of named) {
        // A body may point across at another skill's reference by that skill's
        // name — reflect-edit names orchestrate's writing.md and the comment
        // skill's forms.md — so a named file may ship with any skill.
        expect(
          [
            ...basenames(references),
            ...basenames(orchestrateReferences),
            ...basenames(commentReferences),
          ],
          `the body points at references/${name}, which does not ship`,
        ).toContain(name);
      }
    }
  });

  it("keeps the tier table in SKILL.md and declares it nowhere else", () => {
    expect(readWeightLevels(skillFile).map(({ key }) => key)).toEqual([
      "light",
      "standard",
      "heavy",
    ]);
    for (const { label, body } of [...orchestrateReferences, ...reflectEditReferences]) {
      expect(readWeightLevels(body), `${label} declares a second tier table`).toEqual([]);
    }
  });

  it("keeps the moved sections out of the body", () => {
    expect(skillFile).not.toContain("\n## Writing a document");
    expect(skillFile).not.toContain("\n## Worked example");
    expect(reflectEditFile).not.toMatch(/\*\*Worked, end to end\.\*\*/);
  });

  it("caps both bodies so the shrink cannot quietly regrow", () => {
    // Whole file, INFRA-038's own count (bytes ÷ 4 ≈ tokens). The reflect-edit
    // cap is the 4,000-token error line the AGENT-066 debt named.
    expect(Buffer.byteLength(skillFile, "utf8")).toBeLessThan(51_000);
    expect(Buffer.byteLength(reflectEditFile, "utf8")).toBeLessThanOrEqual(16_000);
  });
});
...
describe("a listener launched at its designation's weight", () => {
  const body = orchestratePackage;
  const skillFile = documentAt("claude/skills/orchestrate/SKILL.md").body;
  // The Routing decisions plus the launch procedure they point at (AGENT-067).
  const routing =
    skillFile.slice(skillFile.indexOf("## Routing"), skillFile.indexOf("## Delegation")) +
    "\n\n" +
    orchestrateReference("launching.md");
  const procedure = orchestrateReference("launching.md");
  const launch = procedure.slice(
    procedure.indexOf("- **Launching a listener.**"),
    procedure.indexOf("- **Losing a listener.**"),
  );
  const losing = procedure.slice(
    procedure.indexOf("- **Losing a listener.**"),
    procedure.indexOf("- **A lane that already has a listener"),
  );
  const occupied = procedure.slice(
    procedure.indexOf("- **A lane that already has a listener"),
    procedure.indexOf("- **A lane with work waiting and nobody on it gets a listener"),
  );
  const roster = procedure.slice(
    procedure.indexOf("- **A lane with work waiting and nobody on it gets a listener"),
    procedure.indexOf("- **A row that does not read `live`"),
  );
  ...
  it("resolves the designation's weight through the tier table", () => {
    ...
    expect(launch).toMatch(
      /carries a \*\*Key\*\* from the tier table in the skill body's Delegation/,
    );
  ...
  it("judges a weightless designation on the conversation, at launch", () => {
    ...
    expect(launch).toMatch(
      wrapped(
        "the two passes in the skill body's Delegation govern **dispatching a job**, and they do not govern **launching a listener**.",
      ),
    );
  ...
  it("hands an unmeetable weight to the listener in words, as well as to the log", () => {
    ...
    expect(launch).toMatch(/`references\/weight\.md` gives the three causes and the rule/);
...
describe("profile skill body", () => {
  ...
  /**
   * A value the skill tells the agent to write to a file, read out of the fenced
   * block that shows the file's contents (AGENT-058).
   *
   * It used to read the value out of a captured heredoc. The construction is
   * gone — a value somebody else wrote goes in by path now — so the example
   * shows the *file* instead, and this reads that. What it protects is
   * unchanged: the worked example's values are checked against the rules stated
   * above them, and a prose edit that drops one still fails here.
   */
  const fileValue = (path: string): string | undefined =>
    new RegExp(
  ...
  it("teaches the create as the whole profile and the description as the judgement", () => {
    expect(body).toMatch(/corpus doc create --type agent-def --flag-file title=/);
    expect(body).toMatch(/--flag-file extra=\S+/);
  ...
  it("works an example description written as when to reach for the agent", () => {
    ...
    // The example shows the file's contents, and `--extra` takes a `key=value`
    // pair — so the description is what follows `description=` in that block.
    const description = /```\ndescription=([\s\S]*?)\n```/.exec(worked)?.[1];
  ...
  it("routes every word a person reads through a heredoc, not a quoted argument", () => {
    expect(body).toMatch(/\*\*Both values below are somebody else's, so both go in by path\.\*\*/);
    ...
    expect(body).toMatch(/binds both of\s+them every time, the ones that look safe included/);
    ...
    for (const line of commandLines) {
      ...
      expect(line, "a value somebody wrote is quoted into the command again").not.toMatch(
        /--(?:title|description) "/,
      );
    }
    ...
    for (const line of creates) expect(line).toMatch(/--flag-file title=\S+/);
    const descriptionEdits = commandLines.filter((line) => line.includes("--flag-file extra="));
    expect(
    ...
    const worked = body.slice(body.indexOf("## Worked example"));
    // Read out of the file the example shows, not a heredoc capture (AGENT-058).
    const description = /```\ndescription=([\s\S]*?)\n```/.exec(worked)?.[1];
  ...
  it("works an example persona that obeys the body rules above it", () => {
    ...
    const profileBody =
      /--type agent-def --flag-file title=\S+ --from agent <<'CORPUS_EOF'\n([\s\S]*?)\nCORPUS_EOF/.exec(
        worked,
      )?.[1];
...
describe("one rule, one skill", () => {
  ...
  const skillBody: Record<SkillName, string> = {
    orchestrate: orchestratePackage,
      ...
      pointers: [
        {
          skill: "comment",
          carries: wrapped(
            "**What the shell does to a flag argument, and why a path is the answer, is the orchestrate skill's to state, and it is stated there alone.**",
          ),
        },
        {
          skill: "converse",
