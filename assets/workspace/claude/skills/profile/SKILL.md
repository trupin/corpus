...
Write `/tmp/corpus-title-bookkeeper.txt` with your file-writing tool — named for the profile:
...
```bash
corpus doc create --type agent-def --flag-file title=/tmp/corpus-title-bookkeeper.txt --from agent <<'CORPUS_EOF'
...
Write `/tmp/corpus-extra-bookkeeper.txt` — named for the flag and the profile — which holds
...
```bash
corpus doc edit doc_b7c1d5 --flag-file extra=/tmp/corpus-extra-bookkeeper.txt --from agent
```
...
tool — `/tmp/corpus-title-bookkeeper.txt`:

```
Bookkeeper
```

and `/tmp/corpus-extra-bookkeeper.txt`, which holds the whole `extra` pair:
