...
- `GET /api/docs?q=&type=&status=&includeArchived=&tag=&folder=&parent=&references=&agent=&author=&since=&due=&stale=&unread=&needs=&sort=` — **the single collection query endpoint** behind every list: structured filters compose with optional FTS (`q`, matching titles/bodies/turns, returning snippet highlights). Thread-specific filters (`parent`, `agent`, `unread`) no-op for non-thread types. There is no separate awaiting-reply filter: threads awaiting a form answer are queried via `needs=form`. Each Attention reason is individually addressable (`needs=unread-reply|form|due|stale|failed-job`), and `needs=me` is their union: unread agent replies ∪ unanswered forms ∪ due/overdue ∪ stale-for-review ∪ failed jobs.
