...
- **Local-first by default** _(user decisions, 2026-07-30 and 2026-07-31)_. Semantic indexing never requires a network account or an API key: the server uses a local model runtime already present on the machine when one is reachable. With no local runtime and no configured provider, semantic indexing reports itself **disabled** — search stays lexical and says so — rather than the tool bundling a model (a bundled runtime would multiply the install footprint ~100×; measured and rejected 2026-07-31). Corpus remains a small local tool out of the box. An external embedding provider may be configured explicitly in `.corpus/config.json`.
