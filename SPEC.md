...
- **Projection integrity is checkable and reconstructible.** `corpus db doctor` fails when files and projection rows drift; `corpus db rebuild` reconstructs the projection from files alone. `rebuild && doctor` clean is the standing invariant. `corpus db doctor` may also carry **report-only warnings** — findings worth a person's attention that are not drift. They never affect its verdict or its exit code.
