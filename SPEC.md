...
4. **The CLI is a thin typed client.** It performs no document writes — every mutation of workspace content is an API call through the generated typed client (§9.3). The two exceptions, both outside the server's ownership, are `corpus init` (which exists precisely to create the workspace the server then owns) and the server-lifecycle runtime state the CLI itself manages (§2.1: the pidfile and logfile under `.corpus/` that `corpus server start|stop` create, clean, and remove — process supervision cannot go through the process being supervised). The agent interacts with the system **only through the CLI**, so file formats are parsed and serialized in exactly one place: the server.
