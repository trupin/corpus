...
| `["docs", "<docId\|threadId>", "cost"]` | `docCostKey(id)`        | — (under `docKey(id)`. **Telemetry announces nothing of its own** (SPEC.md §9.4): a `corpus` invocation emits no frame, so this refreshes only on frames that already exist, and the panel is knowingly not live to the ledger)                                                                                                                                                                                                                                                                                                                         | `useDocCost(id)`                                                                                                        |
...
The three document-scoped shapes are the kit's own for a different reason: the
contract's vocabulary is closed, and ranked search, a related set and a cost
series need no name of their own in it. All three hang **under the `["docs"]`
prefix** the server already emits on every mutation, so prefix matching
invalidates them for free — a `[[ref]]` appearing anywhere refreshes an open
related panel with no new frame, no contract change and no artifact regeneration
(sprint-022 Open Conflict 7).

`["docs", id, "cost"]` hangs there for that reason **and one of its own**, worth
stating because it is a design choice rather than an oversight. SPEC.md §9.4's
telemetry announces nothing: a frame per `corpus` invocation would put the
telemetry channel on the hot path it exists to measure (sprint-025 R8). So no
`invalidate` will ever name this entry directly, and hanging it under `["docs"]`
is the only thing that refreshes it at all.
