...
const BOX_PROPERTIES = ["width", "max-width", "height", "box-sizing"] as const;
...
describe("the reserved image box", () => {
  ...
  it("states a retunable width and height rather than a bare cap", () => {
    ...
    expect(box.get("width")).toBe("var(--md-img-box-w, 240px)");
    expect(box.get("height")).toBe("var(--md-img-box-h, 180px)");
  });
