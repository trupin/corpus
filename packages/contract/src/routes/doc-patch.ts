...
export const patchDoc = createRoute({
  ...
  description:
    ...
    "**It presents no key** (SPEC.md §7). It names the text it expects to find, which is the same " +
    "staleness check by another route — and a better one here, since a patch whose text has moved " +
    "is told *which* text is gone rather than *that* the document changed. Requiring a key would " +
    "also refuse a perfectly well-anchored patch because an unrelated paragraph moved.\n\n" +
