...
export interface paths {
    ...
    "/api/threads/{id}/digest": {
        ...
        get?: never;
        /**
         * Write the thread's digest
         * @description Records a **digest** on the thread: a short prose account of the conversation so far, written by the thread's resident (SPEC.md §6, rider signed 2026-09-05). One per thread — a write replaces whatever was there, and there is no history of them on this surface. The digest reads back on `GET /api/threads/{id}` and in the thread's context pack.
         *
         *     **The body carries prose and nothing else. The server stamps the watermark**, which is the timestamp of the newest turn the thread holds at the instant of the write. There is no `watermark` field to send, and sending one is a `400` naming the key: only the server knows what the newest turn is at that instant, so a watermark chosen by the writer is a claim about a moment that has already passed — it would over-claim coverage of a turn the digest never saw. The response carries the stamped value, so the writer never has to re-read the thread to learn what it just wrote.
         *
         *     **A write clears `stale`.** Staleness means a turn at or before the watermark was deleted or revised since the digest was written, and the rider makes rewriting the digest the only thing that corrects it — the server records staleness and repairs nothing.
         *
         *     **Refusals.** `404` when the thread is unknown, or when the id names a document that is not a thread. `422` in two cases, told apart at `code`: `unknown_recipient` when the thread holds **no resident**, because a digest is the resident's and a thread without one may not have one; `bad_request` when the body is **blank**, because clearing a digest is `DELETE /api/threads/{id}/digest` and an empty write is not a clear. Both are `422` rather than `400` because retrying this route with a different body cannot help in either case. A body longer than 2000 characters is an ordinary `400`, since a shorter one is exactly what would work.
         *
         *     **One action, one commit** (SPEC.md §4), authored by the acting party: the write rewrites the thread file's frontmatter and auto-commits it, so the response carries §11's warnings — a workspace hook that rejects the commit leaves the digest on disk and uncommitted. It presents no key (SPEC.md §7): it writes one frontmatter field and replaces nothing a reader was holding.
         */
        put: {
            parameters: {
                query?: never;
            ...
            requestBody: {
                content: {
                    "application/json": components["schemas"]["WriteDigestRequest"];
                };
            };
            responses: {
                /** @description The digest as stored — the body as written, the server's stamped `watermark`, and `stale` false — plus any warnings raised while writing it. */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["ThreadDigestResponse"];
                    };
                };
                /** @description The request failed schema validation; `issues` names the offending fields. */
                400: {
                ...
                /** @description The write was refused and nothing was written. `unknown_recipient` — the thread holds no resident, and a digest is the resident's (SPEC.md §6); `recipient` carries the thread id, and the remedy is `POST /api/threads/{id}/resident`. `bad_request` — the body is blank, and an empty write is not a clear: `DELETE /api/threads/{id}/digest` removes a digest. */
                422: {
                    headers: {
                        [name: string]: unknown;
                    };
                    content: {
                        "application/json": components["schemas"]["ValidationError"] | components["schemas"]["UnknownRecipientError"];
                    };
                };
            };
        };
        post?: never;
        /**
         * Clear the thread's digest
         * @description Removes the thread's digest, leaving the thread in the state the rider calls ordinary: **a thread with no digest is not a fault** (SPEC.md §6). This is the only way to clear one — an empty `PUT` is refused rather than read as a clear, so a resident that meant to update a digest can never destroy it by sending nothing.
         *
         *     **Idempotent.** Clearing a thread that has no digest is a `200` that changes nothing, writes nothing and commits nothing. It answers with the same shape the write does, its `digest` null, because a clear that *does* write can raise §11's warnings and a rejected auto-commit has to be visible somewhere.
         *
         *     `404` when the thread is unknown. `422` when the thread holds no resident — the same refusal the write makes, because a thread that may not have a digest may not be asked about one either, and answering `200` there would tell a caller its clear had taken effect on a surface that never had anything to clear.
         *
         *     It presents no key (SPEC.md §7): it clears one frontmatter field and replaces nothing a reader was holding.
         */
        delete: {
            parameters: {
                query?: never;
            ...
            requestBody?: never;
            responses: {
                /** @description The thread, its `digest` now null, and any warnings raised while clearing it. Unchanged when there was no digest to clear. */
                200: {
                    headers: {
                        [name: string]: unknown;
                    };
                ...
                /** @description The thread holds no resident, so it has no digest to clear (SPEC.md §6): a digest belongs to a resident, and a thread without one never has one. Nothing was written. `recipient` carries the thread id — the same code and the same field the write is refused with, because it is the same fact about the same thread. */
                422: {
                    headers: {
                        [name: string]: unknown;
                    };
    ...
    "/events": {
         ...
         *     - `["threads", "<threadId>"]` — emitted by thread creation, turn append, turn deletion, resolve/reopen, mark-seen, and a digest write or clear for that thread. Refetch: `GET /api/threads/{id}` — the open thread view and its unread badge.
...
export interface components {
    schemas: {
        ...
        Thread: {
            ...
            /** @description The thread's digest — a short prose account of the conversation so far, written by the thread's **resident** and never by the server (SPEC.md §6, rider signed 2026-09-05) — or **null**, which is the ordinary state and not a fault. One per thread: a second write replaces the first, and there is no history of them here. A thread with no resident never has one, because writing it is refused on such a thread. Read `stale` before quoting anything from it, and read the conversation itself before quoting anything at all: a digest orients, and an agent quotes and edits from turns it has read verbatim. */
            digest: components["schemas"]["ThreadDigest"] | null;
        ...
        ThreadDigest: {
            /** @description **The digest's prose, exactly as its author wrote it** (SPEC.md §6, rider signed 2026-09-05). Markdown, a short account of the conversation so far. Written by the thread's resident through `PUT /api/threads/{id}/digest` and **by nothing else**: the server never generates it, never edits it and never repairs it, because a summary nobody wrote is worse than none. Writes are bounded at 2000 characters (`DIGEST_MAX_CHARS`); this field publishes no ceiling of its own, since the only way to bring an over-long value inside one would be to cut it, and cutting it is editing it. **It orients, and it is not a source**: an agent quotes and edits from turns it has read verbatim, never from this. */
            body: string;
            /**
             * Format: date-time
             * @description **The timestamp of the newest turn this digest covers** (SPEC.md §6). A turn's `ts` — the turn's identity within the thread — and always one that existed when the digest was written. **Stamped by the server**, never by the writer: only the server knows what the newest turn is at the instant of the write, so the write request carries no watermark field at all. Turns newer than this are **not yet covered**, which is the ordinary state between a message landing and the resident's next reply — it is not staleness, and it does not make the digest wrong about what it does cover.
             * @example 2026-07-19T10:05:00Z
             */
            watermark: string;
            /** @description **Whether a turn at or before `watermark` was deleted or revised since** (SPEC.md §6), so this account covers text that is no longer there. `true` is the server *recording* that, and it changes nothing else: the body is left exactly as written, because repairing it is the resident's act and never the server's. A stale digest **is shown as stale wherever it is shown**, which is why the flag rides inside the digest object rather than beside it — a surface cannot carry the prose without carrying this. It clears only when a resident writes the digest again. **Not the same as uncovered**: turns *after* the watermark leave this `false`, and are read from `watermark` instead. */
            stale: boolean;
        };
        Turn: {
            /**
        ...
        ThreadDigestResponse: {
            /**
             * @description The thread whose digest this is.
             * @example th_x9y8
             */
            threadId: string;
            /** @description The thread's digest **after this call**: the stored digest — its `watermark` stamped by the server and its `stale` cleared — after a write, and **null** after a clear, including a clear that had nothing to clear. Read the watermark from here rather than guessing it: the writer never states one. */
            digest: components["schemas"]["ThreadDigest"] | null;
            /** @description Non-fatal problems noticed while performing this mutation (SPEC.md §11), **and effects it had on documents it was not asked to act on** (§7's skill folder move; CONTRACT-047). The mutation succeeded regardless — files are the source of truth and the server never rolls a write back because a commit or a check failed, and a carried effect is not a failure at all but a consequence the caller is owed. **This is a reporting channel and not a severity class**: its members run from `carried_skill`, where nothing went wrong at all, to `commit_failed`, and `validation_error` carries a §11 finding of error severity that the save reported rather than refused (CONTRACT-084). Read `code` to tell them apart, never the position in the array and never `detail`. Empty when nothing went wrong and the act touched nothing beyond what it was asked to do. */
            warnings: components["schemas"]["Warning"][];
        };
        WriteDigestRequest: {
            /** @description The digest's prose — markdown, a short account of the conversation so far. At most 2000 characters (`DIGEST_MAX_CHARS`); longer is a `400` naming this field. **Blank is refused, not honoured**: an empty or whitespace-only body is a `422`, because clearing a digest is `DELETE /api/threads/{id}/digest` and an empty write is not a clear. **There is no `watermark` field on purpose** — the server stamps the newest turn's timestamp at the instant of the write, which is the only moment at which it can be stated truthfully. A write replaces whatever digest the thread had and clears its `stale` flag. */
            body: string;
        };
        AgentRoster: {
