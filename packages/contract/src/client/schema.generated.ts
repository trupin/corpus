...
export interface paths {
    ...
    "/api/threads/{id}/resident": {
         ...
         *
         *     **Naming a profile is the refinement.** `name`, when given, is the invocable name `@<subagent>` mentions already use (SPEC.md §8) — for a `type: agent-def` document **under `.claude/agents/`**, its filename stem or its title, case-insensitively, never a document id — and is how a conversation gets an agent that behaves differently from the default. `404` when the thread is unknown, and `404` when the name resolves to no such document: a name that misses is refused rather than degraded to a general resident, because a typo that looked like it worked is the worse outcome. An `agent-def` filed outside that root is one of those misses — it is a document *about* a persona, nothing loads it as a subagent, and it answers to neither spelling — so the refusal names its path, moving the file into the root being what makes it designatable. A **blank** name (`""`, `"   "`) is a `400` and not absence, for the same reason.
         *
