...
export interface components {
    schemas: {
        ...
        BulkActionResult: {
            ...
            /** @description Documents the act changed — §11's first part, and exactly the files in `commit` (§4). Empty is a legal outcome: every document was already in the target state, or every one was refused. */
            changed: string[];
            /** @description Documents that were **already in that state** — §11's second part, explicitly a no-op and **not a failure**: "a document already archived is a no-op, not a failure". They contribute nothing to the commit, and a board must not colour them as errors. The `review` act populates it only when the instant it would write is the one already there: instants are second-precision, so repeating `review` on the same document inside one second genuinely moves no bytes. Reporting it as changed would put an id in `changed` that `git show --name-only` does not list, and that equality is the stronger, testable invariant (SERVER-077). */
