...
export interface paths {
    ...
    "/api/threads/{id}/turns": {
         ...
         *
         *     **A form fence in the body is validated here.** Forms are written only through this endpoint (SPEC.md §6), so a turn whose ```` ```form ```` block does not parse against the grammar — unreadable YAML, a fourth field kind, duplicate questions, a duplicate option, a `write` field carrying `options` — is a `400` and never reaches disk. Refusing at write time is what makes the reader's rule (§11: an unreadable form renders as the visibly broken code block it is, never as a partial set of controls) a safety net for bytes that arrived some other way, rather than the only line of defence.
         */
        post: {
            parameters: {
                query?: never;
