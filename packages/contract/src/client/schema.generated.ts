...
export interface paths {
    ...
    "/api/capture": {
         ...
         *
         *     **The filing thread is created with a resident, unless the caller says otherwise** (SPEC.md §10, rider signed 2026-08-25). Omitting `resident` designates a general resident; `null` creates the thread with none; an object designates a profile. **A capture carries a designation although it carries no `recipient`**, and the rider says why: the reason it has no recipient — a capture creates a standalone thread in no scope by construction — is about *routing*, not about *ownership*. There is nowhere to route a message before there is a conversation, and there is very much a conversation to own.
         */
        post: {
            parameters: {
                query?: never;
