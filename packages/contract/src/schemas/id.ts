...
export const DocIdSchema = z
  .string()
  .regex(prefixed("doc"))
  .openapi({ description: "Identifier of a non-thread document.", example: "doc_a1b2c3" });

export const ThreadIdSchema = z
  .string()
  .regex(prefixed("th"))
  .openapi({ description: "Identifier of a thread document.", example: "th_x9y8" });
...
export const DocumentIdSchema = z
  .string()
  .regex(/^(doc|th)_[A-Za-z0-9]+$/)
  .openapi({
    description: "Identifier of any document; threads are documents too.",
    example: "doc_a1b2c3",
  });

export const AnchorIdSchema = z.string().regex(prefixed("anc")).openapi({
...
export const EventIdSchema = z
  .string()
  .regex(prefixed("evt"))
  .openapi({ description: "Identifier of a queue event.", example: "evt_7c1d" });
