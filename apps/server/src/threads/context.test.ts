...
/**
 * SERVER-144. The audit measured **4 of 11 excerpt rows** in one pack naming
 * `doc_skillcomment`, `doc_skillorchestrate`, `doc_skillconverse` and
 * `doc_skillb8a2308c` — the agent's own instructions, quoted back to it as
 * though they were the corpus, at roughly 580 tokens an event.
 */
describe("the pack's excerpts are the corpus, not the machinery (SERVER-144)", () => {
  ...
  beforeAll(() => {
    ...
    machinery.doc({ id: "doc_tpl01", type: "template", title: "Template", body: "Escrow." });
    machinery.doc({ id: "doc_view01", type: "view", title: "A view", body: "Escrow." });
    machinery.doc({ id: "doc_brd01", type: "board", title: "A board", body: "Escrow." });
    machinery.doc({ id: "doc_peer01", title: "Peer note", body: "The escrow reserve is held." });
    machinery.doc({
      id: "doc_parent01",
      title: "Mortgage",
  ...
  it("carries no excerpt naming an excluded type, though every one is linked", async () => {
    ...
    expect(result.excerpts.map((row) => row.id)).toEqual(["doc_peer01"]);
  });
