 ...
 * keep. This is the one bump so far that removes rather than adds, and the wipe
 * is exactly the right migration for it: an upgrading workspace has a populated
 * `locks` table and a `.corpus/locks/` directory full of leases, and the rebuild
 * this bump triggers drops the whole table with the database it lived in. A key
 * is derived from the document and the editing signal is in memory, so nothing
 * replaces the rows.
 */
