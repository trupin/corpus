...
/**
 * SERVER-144. Measured on a fresh workspace in the SHARED-070 audit: the #1
 * related document for a user's mortgage note was `doc_skillorchestrate`.
 *
...
describe("the product's own machinery is never a neighbour (SERVER-144)", () => {
  ...
  beforeAll(() => {
    ...
    machinery.doc({ id: "doc_peer01", title: "Peer note", body: "An ordinary neighbour." });
    machinery.doc({
      id: "doc_skill01",
  ...
  it("answers with the corpus and not with the machinery", async () => {
    expect(await neighbours("doc_subject")).toEqual(["doc_peer01"]);
  });
