...
export const createCommand: WorkspaceCommandSpec = {
  ...
  description:
    ...
    "folder the server rejects is reported verbatim rather than pre-validated here. Prints the " +
    "new id and path; `--json` emits the server's `{doc, warnings}` response unchanged.",
  args: [],
  flags: [
    {
      name: "type",
